...
mod_mixfish_plot_display_server <- function(id, plot_name, selected_ecoregion, selected_subRegion) {
  moduleServer(id, function(input, output, session) {
    ...
    #   tmp <- strsplit(data_comp$metier, ".", fixed = TRUE)
    #   data_comp$area <- unlist(lapply(tmp, FUN = function(x) {
    #     ifelse(length(x) == 2, x[2], NA)
    #   }))
    #   tmp <- strsplit(data_comp$fleet, "_", fixed = TRUE)
    #   data_comp$country <- unlist(lapply(tmp, FUN = function(x) {
    #     ifelse(length(x) == 2, x[1], NA)
    #   }))
    #   # replace stock with ICES stock code
    #   data_comp$stock <- refTable$stock[match(data_comp$stock, refTable$stock_short)]
