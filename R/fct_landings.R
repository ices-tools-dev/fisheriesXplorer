...
plot_catch_trends_plotly <- function(
  ...
  # --- NEW: include "Total" rows in the returned data when type == "Fisheries guild"
  if (return_data) {
    if (type == "Fisheries guild") {
      totals <- df %>%
        dplyr::group_by(Year) %>%
        dplyr::summarise(typeTotal = sum(Value, na.rm = TRUE) / 1000, .groups = "drop") %>%
        dplyr::mutate(type_var = "Total")
      plot_df <- dplyr::bind_rows(
        plot_df,
        totals[, c("type_var", "Year", "typeTotal")]
      )
    }
    return(plot_df)
  }
  ...
  file_stub <- paste0(ecoregion,"_landings_", sanitize_stub(type), "_", date_stamp)

  p <- plotly::plot_ly(plot_df, x = ~Year, y = ~typeTotal, color = ~type_var, colors = pal) %>%
    plotly::add_trace(type = "scatter", mode = "lines", line = list(width = 3))

  # --- NEW: add a "Total" line when plotting Fisheries guild
  if (type == "Fisheries guild") {
    total_df <- df %>%
      dplyr::group_by(Year) %>%
      dplyr::summarise(typeTotal = sum(Value, na.rm = TRUE) / 1000, .groups = "drop") %>%
    ...
    p <- p %>%
      plotly::add_trace(
        ...
        line = list(width = 4, color = "black", dash = "dot"),
        hoverlabel = list(font = list(size = base_size)),
        inherit = FALSE
      )
  }
